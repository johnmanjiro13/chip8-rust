...
mod tests {
    ...
    #[test]
    fn with_rom() {
        let rom = vec![0x00, 0x01, 0x02, 0x03, 0x04];
        let want = rom.clone();
        let memory = Memory::with_rom(rom);
        assert_eq!(memory.at[0x200..0x205], want);
    }
    ...
    proptest! {
      ...
      fn load_stored_value(addr in 0x200u16..MEMORY_SIZE as u16 -1, value: u8){
        ...
        assert_eq!(memory.load(addr), value);
      }
      ...
      fn load_overwritten_value(addr in 0x200u16..MEMORY_SIZE as u16 -1, old: u8, new: u8){
        ...
        assert_eq!(memory.load(addr), new);
      }
      ...
      fn load_stored_sprite(from in 0x200u16..MEMORY_SIZE as u16 -9, value: u8) {
        ...
        let got = memory.load_sprite(from, 8);
        assert_eq!(got, want);
      }
      ...
      fn load_font_sprite(font in 0x0u8..=0xFu8) {
        ...
        let want = &FONT[from..from+FONT_SIZE as usize];

        let got = memory.load_sprite(Memory::font_addr(font), FONT_SIZE);
        assert_eq!(got, want);
      }
    }
}
